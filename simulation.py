...
class Robot(pygame.sprite.Sprite):
    ...
    def target(self):
        idx = self.rect.collidelist(list_rect_targets)
        if idx != -1:  # if collision exists
            list_targets.pop(idx)
            list_rect_targets.pop(idx)
        # for target in list_targets:
        #     if self.rect.center[0]>= target.x_topleft and self.rect.center[1] <= target.y_topleft:
        #         self.c_targets += 1
...
def init_simulation(obstacles=10,ambiente=0):
    pygame.init()  
    pygame.display.set_caption('PyWeekend Express - 5to Hackathon ESPOL 2020')
    ...
    if ambiente != 0:
        create_ambiente(ambiente, list_obstacles)
        ...
        for i in range(int(obstacles/2)):
            ...
            list_obstacles.append(obs)
        #list_targets.append((300, 300))
        #list_targets.append((1150, 20))
        target = Obstacle(1150,20,20,20,target_color)
        list_targets.append(target)

    for ob in list_targets:
        list_rect_targets.append(pygame.Rect(ob.x_topleft,ob.y_topleft,ob.width,ob.height))
...
def display_obstacles():
    ...
    for i in range(len(list_targets)):
        s = pygame.display.get_surface()
        s.fill(list_targets[i].color, list_rect_targets[i])
